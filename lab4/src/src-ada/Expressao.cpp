  ...
  */
    ...
    */
    ...
    // Regra 59) aritC -> ID 
        ...
        No_arv_parse* no_call = no->filhos[0];
        
        ExpressaoChamada* res = new ExpressaoChamada();
        res->nome_funcao = ID::extrai_ID(no_call->filhos[0]);

        // A chamada pode ter argumentos (regra 14) ou não (regra 15)
        if (no_call->filhos.size() > 2) { // Tem argumentos
            res->argumentos = extrair_lista_argumentos(no_call->filhos[2]);
        }
...
vector<Expressao*> Expressao::extrair_lista_argumentos(No_arv_parse* no_args) {
    vector<Expressao*> lista;
    
    // Regra: args -> arg, args (recursivo) ou args -> arg (base)
    while (true) {
        No_arv_parse* no_arg = no_args->filhos[0]; // Pega o 'arg'
        // Regra: arg -> arit
        lista.push_back(Expressao::extrai_expressao(no_arg->filhos[0]));

        if (no_args->filhos.size() > 1) { // Caso recursivo: tem ', args'
            no_args = no_args->filhos[2]; // Move para o próximo nó 'args'
        } else {
            break; // Caso base, fim da lista
        }
    }
    return lista;
}

void Expressao::descobrir_variaveis_usadas(std::vector<std::string>& lista_vars){}

void Expressao::calcular_max_params_saida(int& max_atual){}
